  ...
  <nav class="primary flex md:flex-row flex-col h-full w-full">
    ...
    <section class="primary-menu md:flex md:visible invisible flex-row sm:items-center w-full justify-end ml-auto">
      @foreach ($menu as $nav)
          ...
          @if (isset($nav['page']['sub_menu']))
            {{ 'is set' }}
            <div id="menu-link-{{ $nav['page']['post_name'] }}" class="{{$nav['page']['post_name']}} md:py-7 menu-link w-auto relative relative top-0 left-0 lg:mx-6 md:mx-2">
              <a class="py-7 dib font-wide w-full uppercase font-normal lg:text-sm text-xs text-tan hover:opacity-75 tracking-wide" href="/{{ $nav['page']['post_name'] }}">
                {{ $nav->post_title }}
              </a>
            </div>
